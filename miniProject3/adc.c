...
{
	/* Use internal 2.56V and external capacitor at AREF pin */
	ADMUX = (ADMUX & 0x3F) | ((Config_Ptr -> ref_volt )<<REFS0);

	/* ADCSRA Register Bits:
	 * ADEN    = 1 to Enable ADC
	 * ADIE    = 0 to Disable ADC Interrupt
	 * ADATE   = 0 to Disable Auto Trigger
	 * ADPS2:0 = 011 to choose ADC_Clock=F_CPU/8=1Mhz/8=125Khz the ADC must operate in range 50-200Khz
	 */
	ADCSRA |= (1<<ADEN);
